...
public class InvTypeTable {
	...
	private static final String SELECT_ALL_SQL = "SELECT `typeId`,`groupId`,`typeName`,"
			+ "`description`,`mass`,`volume`,`published`,`marketGroupId` FROM invtype";
	...
	public static List<InvType> getAll(Connection db) throws SQLException {
		PreparedStatement stmt = db.prepareStatement(SELECT_ALL_SQL);
		ResultSet rs = stmt.executeQuery();
		List<InvType> types = new ArrayList<InvType>();
		while (rs.next()) {
